...
export default async function CampaignPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10"><div className="mb-8 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-400 p-7 text-white"><p className="text-sm font-semibold uppercase tracking-wider">Special offer</p><h1 className="mt-1 text-3xl font-bold">{campaign.name}</h1>{campaign.description && <p className="mt-2 max-w-2xl text-orange-50">{campaign.description}</p>}</div><div className="products-carousel">{campaign.products.map((product) => <div key={product.id} className="product-card-item"><Link href={`/product/${product.slug}`} className="block overflow-hidden rounded-xl border bg-white p-3 hover:shadow-md"><div className="aspect-square overflow-hidden rounded-lg bg-gray-100">{product.main_image && <img src={product.main_image} alt={product.name} className="h-full w-full object-cover" />}</div><h2 className="mt-3 line-clamp-1 font-semibold text-gray-900">{product.name}</h2><div className="mt-1 flex items-center gap-2"><span className="font-bold text-orange-600">৳{product.price.toLocaleString("en-BD")}</span><span className="text-xs text-gray-400 line-through">৳{product.original_price.toLocaleString("en-BD")}</span></div></Link></div>)}</div></main>;
}
